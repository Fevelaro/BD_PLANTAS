...
fieldnames = ['ID','Nombre','Propiedades','Descripción','Uso','Farmacodinamia','Recomendaciones','Presentación','Antecedentes','Efectos','Precauciones'] #campos
writer = csv.DictWriter(csv_file_info, fieldnames=fieldnames)
writer.writeheader()
for i in range(103):
	name=str(plantas_list['N_com1'][i])
	resp=abrir_libro_planta(i,name)
#	input(resp)
	inf=extraccion_info(resp)
	datos=[[i],[name],[inf[0]],[inf[1]],[inf[2]],[inf[3]],[inf[4]],[inf[5]],[inf[6]],[inf[7]],[inf[8]],[inf[9]]]##Me genera dudas esto
	writer.writerow({'ID':datos[0],'Nombre':datos[1],'Propiedades':datos[2],'Descripción':datos[3],'Uso':datos[4],'Farmacodinamia':datos[5],'Recomendaciones':datos[6],'Presentación':datos[7],'Antecedentes':datos[8],'Efectos':datos[9],'Precauciones':datos[10]})	
